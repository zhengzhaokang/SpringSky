...
import com.wc.single.sky.common.core.text.Convert;
import com.wc.single.sky.common.exception.BusinessException;
...
import com.wc.single.sky.common.utils.ToolUtil;
import com.wc.single.sky.common.utils.reflect.ReflectUtils;
...
public class ExcelUtil<T> {
    ...
    public List<T> importExcel(String sheetName, InputStream is) throws Exception {
        ...
        if (rows > 0) {
            ...
            //表头和类属性的数量 不相等,则报错
            if (fieldsMap.size() != sheet.getRow(0).getLastCellNum()) {
                throw new QdpException(DefaultErrorMessage.getErrorMessage(DefaultErrorMessage.EXCEL_IMPORT_ERROR, ""), DefaultErrorMessage.EXCEL_IMPORT_ERROR.intValue());
     ...
     * @throws Exception
     */
    public Map<String, String> getExcelHeader(String sheetName, InputStream is) throws Exception {
        this.type = Excel.Type.IMPORT;
        this.wb = WorkbookFactory.create(is);
        if (StringUtils.isNotEmpty(sheetName)) {
            // 如果指定sheet名,则取指定sheet中的内容.
     ...
     * @throws Exception
     */
    public Integer getCount(InputStream is, String... sheetNames) throws Exception {
        this.type = Excel.Type.IMPORT;
        this.wb = WorkbookFactory.create(is);
        int count = 0;
        for (String s : sheetNames) {
            ...
            count = count + sheet.getPhysicalNumberOfRows();
        }
        ...
        for (int i = startNo; i < endNo; i++) {
            ...
            for (int j = 0; j < fields.size(); j++) {
                ...
                try {
                    ...
                    log.error("导出Excel失败，异常信息：" + e.getMessage(), e);
                }
            }
        }
    }
     ...
     * @param resultMap
     */
    public static void exportExcel(String name, List<String> headerList, List<? extends BaseEntity> resultMap) {
        exportTable2Workbook(getTwoArrayContent(name, headerList, resultMap), getResponse());
    }
     ...
     * @param baseEntities
     */
    private static SXSSFWorkbook getTwoArrayContent(String name, List<String> headerList, List<? extends BaseEntity> baseEntities) {
        String[][] content = new String[baseEntities.size()][headerList.size()];
        for (int i = 0; i < baseEntities.size(); i++) {
            BaseEntity baseEntity = baseEntities.get(i);
            Map<String, Object> stringObjectMap;
            try {
                stringObjectMap = objectToMap(baseEntity);
            ...
            for (String s : headerList) {
                try {
                    content[i][j] = stringObjectMap.get(s).toString();
        ...
        return getSXSSFWorkbook(name, headerList, content);
    }

    /**
     * 根据tableName 和 where条件导出excel(文件流)
     *
     * @param workbook
     * @param response
     * @return
     * @throws Exception
     */
    private static void exportTable2Workbook(Workbook workbook, HttpServletResponse response) {
        String suffix = EXCEL_2003_SUFFIX;
        ...
        try {
            String fileName = workbook.getSheetName(0) + suffix;
     ...
     * @param values    内容
     */
    public static SXSSFWorkbook getSXSSFWorkbook(String sheetName, List<String> title, String[][] values) {
        ...
        for (int i = 0; i < values.length; i++) {
            ...
            for (int j = 0; j < values[i].length; j++) {
                ...
                try {
                    ...
                    row.createCell(j).setCellValue(new HSSFRichTextString(values[i][j]));
                }
            }
        }
        return wb;
    ...
    public void exportExcel(HttpServletResponse response, List<T> list, String bu数据) {
    }
}